...
(function (document, window) {
	...
	var impress = (window.impress = function (rootId) {
		...
		var buildConfig = function () {
			...
			return {
				width: lib.util.toNumber(
					rootData.width,
					defaults.width
				),
				height: lib.util.toNumber(
					rootData.height,
					defaults.height
				),
				maxScale: lib.util.toNumber(
					rootData.maxScale,
					defaults.maxScale
				),
				minScale: lib.util.toNumber(
					rootData.minScale,
					defaults.minScale
				),
		...
		var getStep = function (step) {
			...
			return step && step.id && stepsData["impress-" + step.id]
				? step
				: null;
		};
		...
		var swipe = function (pct) {
			...
			if (pct < 0) {
				...
				event.detail.next =
					idx < steps.length ? steps[idx] : steps[0];
				event.detail.reason = "next";
			} else if (pct > 0) {
				idx = steps.indexOf(activeStep) - 1;
				event.detail.next =
					idx >= 0 ? steps[idx] : steps[steps.length - 1];
			...
			var interpolatedStep = {
				translate: {
					x: interpolate(
						...
						-nextStep.translate.x,
						k
					),
					y: interpolate(
						currentState.translate.y,
						-nextStep.translate.y,
						k
					),
					z: interpolate(
						currentState.translate.z,
						-nextStep.translate.z,
						k
					),
				},
				rotate: {
					...
					order:
						k < 0.7
							? currentState.rotate.order
							: nextStep.rotate.order,
				},
				scale: interpolate(
					currentState.scale * windowScale,
					nextScale,
					k
				),
			};
			...
			css(root, {
				...
				// we need to 'scale' the perspective, too
				perspective:
					config.perspective / interpolatedStep.scale +
					"px",
	...
	var libraryFactory = function (rootId) {
		...
		var teardown = function () {
			...
			for (i = 0; i < elementList.length; i++) {
				elementList[i].parentElement.removeChild(
					elementList[i]
				);
			}
			elementList = [];
	...
	var libraryFactory = function (rootId) {
		...
		var toNumberAdvanced = function (numeric, fallback) {
			...
			if (ratio == null) {
				...
				var multiplier =
					ratio[2] === "w" ? config.width : config.height;
	...
	var libraryFactory = function (rootId) {
		...
		var rotationToQuaternion = function (baseCoordinate, rotation) {
			...
			for (var i = 0; i < axes.length; i++) {
				...
				if (i > 0) {
					coordinate = {
						x: rotateByQuaternion(
							baseCoordinate.x,
							result
						),
						y: rotateByQuaternion(
							baseCoordinate.y,
							result
						),
						z: rotateByQuaternion(
							baseCoordinate.z,
							result
						),
					};
		...
		var rotateVector = function (baseCoordinate, vec, rotation) {
			var quaternion = rotationToQuaternion(
				baseCoordinate,
				rotation
			);
		 ...
		 */
		var angleBetweenPlaneAndVector = function (
			axis,
			planeVec,
			rotatedVec
		) {
		...
		var bestRotation = function (baseRotate, rotations) {
			...
			for (var i = 0; i < rotations.length; ++i) {
				...
				var score =
					Math.abs(rotation.x - baseRotate.x) +
...
(function (document) {
	...
	document.addEventListener(
		...
		function (event) {
			...
			var autoplay =
				util.getUrlParamValue("impress-autoplay") ||
				data.autoplay;
	...
	var reloadTimeout = function (event) {
		...
		currentStepTimeout = util.toNumber(
			step.dataset.autoplay,
			autoplayDefault
		);
		if (status === "paused") {
	...
	const convertMarkdowns = function (selector) {
		...
		for (var elem of document.querySelectorAll(selector)) {
			...
			for (var i = 0; i < slides.length; ++i) {
				slideElems[i].innerHTML = parseMarkdown(
					slideElems[i],
					slides[i]
				);
			...
			if (keylist.length !== nextlist.length) {
				...
				if (index >= 0) {
					...
					if (isNumber(next)) {
						...
						if (
							newTarget &&
							newTarget.classList.contains("step")
						) {
	...
	var impressConsole = (window.impressConsole = function (rootId) {
		...
		var nextStep = function () {
			...
			while (nextElement) {
				...
				if (classes && classes.value.indexOf("step") !== -1) {
					consoleWindow.document.getElementById(
						"blocker"
					).innerHTML = lang.next;
					return nextElement;
				}
		...
		var onStepLeave = function () {
			if (consoleWindow) {
				...
				// Set the views
				var baseURL = document.URL.substring(
					0,
					document.URL.search("#/")
				);
				var slideSrc =
				...
				var preView =
					consoleWindow.document.getElementById("preView");
		...
		var onStepEnter = function () {
			if (consoleWindow) {
				...
				var notes =
					consoleWindow.document.getElementById("notes");
		...
		var onSubstepShow = function () {
			var slideView =
				consoleWindow.document.getElementById("slideView");
		...
		var spaceHandler = function () {
			...
			if (notes.scrollTopMax - notes.scrollTop > 20) {
				notes.scrollTop =
					notes.scrollTop + notes.clientHeight * 0.8;
			} else {
				window.impress().next();
			}
		};
		...
		var consoleOnLoad = function () {
			...
			// Firefox:
			slideView.contentDocument.body.classList.add(
				"impress-console",
				"slideView"
			);
			preView.contentDocument.body.classList.add(
				"impress-console",
				"preView"
			);
		...
		var resize = function () {
			...
			var preViewWidth = Math.floor(
				slideViewWidth * preViewDefaultFactor
			);
			var preViewHeight = Math.floor(
				slideViewHeight * preViewDefaultFactor
			);
			...
			if (
				preViewWidth <=
				Math.floor(slideViewWidth * preViewMinimumFactor)
			) {
				...
				preViewWidth = Math.floor(
					slideViewWidth * preViewMinimumFactor
				);
				preViewHeight = views.clientHeight - delta - preViewTop;
			}
		...
		var init = function (cssConsole, cssIframe) {
			if (
				(cssConsole === undefined ||
					cssConsole === cssFileOldDefault) &&
				(cssIframe === undefined ||
					cssIframe === cssFileIframeOldDefault)
			) {
				window.console.log(
	...
	onStepenterDetectImpressConsole = function () {
		return {
			preview:
				...
				window.frameElement.id === "preView",
			slideView:
				window.frameElement !== null &&
				window.frameElement.id === "slideView",
		};
	};
	...
	var computeRelativePositions = function (el, prev) {
		...
		if (data.relTo) {
			...
			if (ref) {
				...
				if (
					el.compareDocumentPosition(ref) &
					Node.DOCUMENT_POSITION_PRECEDING
				) {
					prev.x = toNumberAdvanced(
						ref.getAttribute("data-x")
					);
					prev.y = toNumberAdvanced(
						ref.getAttribute("data-y")
					);
					prev.z = toNumberAdvanced(
						ref.getAttribute("data-z")
					);

					var prevPosition =
						ref.getAttribute("data-rel-position") ||
						"absolute";
					...
					if (prevPosition !== "relative") {
						...
						prev.rotate.y = toNumber(
							ref.getAttribute("data-rotate-y")
						);
						prev.rotate.x = toNumber(
							ref.getAttribute("data-rotate-x")
						);
						...
						prev.relative = {
							...
							x: toNumberAdvanced(
								ref.getAttribute("data-rel-x"),
								0
							),
							y: toNumberAdvanced(
								ref.getAttribute("data-rel-y"),
								0
							),
							z: toNumberAdvanced(
								ref.getAttribute("data-rel-z"),
								0
							),
							rotate: {
								x: toNumberAdvanced(
		...
		if (visible.length < substeps.length) {
			...
			for (var j = visible.length; j < substeps.length; j++) {
				if (
					currentSubstepOrder &&
					currentSubstepOrder !==
						substeps[j].dataset.substepOrder
				) {
...
(function (document, window) {
	...
	document.addEventListener("touchend", function () {
		...
		if (
			...
			window.impress().goto(
				document.querySelector("#impress .step.active")
			);
		}
	});
